...
| 放浪部族(Vagabond) | ✅ 実装済み(アイテム3ゾーン・派閥関係/悪名・戦闘読み替え・クエスト・キャラ3種・共闘軍) |

- 秋マップ(12広場・樹林7)・山札54枚・派閥ボード数列は公式コンポーネントから確定済み(`rules/data-verification.md`)
- 勝利手段3種を完備: 30VP到達(3.1)・圧倒カード(3.3)・共闘軍(9.2.8)
- pytest 65件 + 4派閥1000戦スモーク(全ステップ不変量検証付き)がクラッシュゼロ
- 並列対戦ランナー(SQLite保存)+集計+Chart.jsダッシュボードで run 間の勝率比較が可能
- ヒューリスティックbot(1手先読みgreedy+派閥別評価関数)がランダムbotの偏りを是正(猫 1%→78.5%)
- **PPO self-play学習基盤**(`rl/`): 固定行動空間+合法手マスク+AEC互換環境。Mac CPUの50万ステップ学習で猫がヒューリスティックbotに勝率97〜100%到達
- 未実装の主な簡略化: 城砦の配置禁止(6.2.2)、item以外のクラフト効果(immediate/persistent)、猫の行軍2移動・野戦病院、同盟の同時移動/攻撃(9.2.9.II.b〜d)— ロードマップ「フェーズR」で解消中
...
勝利は30VP到達(3.1)のほか、3人戦以上では圧倒カード(3.3)・共闘軍(9.2.8)でも決まる。`--max-turns`(既定300)超過は timeout(引き分け)として記録される。同一シードで結果は完全に再現可能。
...
torch は `.venv/`(gitignore)に隔離し、エンジン本体の依存ゼロは維持したまま。

```bash
# venv 準備(初回のみ)
python3 -m venv .venv && .venv/bin/pip install torch numpy pytest

# PPO self-play 学習(既定: 猫 vs 鷲巣・完全情報)
.venv/bin/python -m rl.train --total-steps 500000 --run-name my-run
tail -f rl_runs/my-run/log.csv          # 進捗(CSVログ)

# 学習済み ckpt の評価(既存の RandomBot / HeuristicBot と対戦)
.venv/bin/python -m rl.eval --ckpt rl_runs/my-run/ckpt_240.pt --games 32
```

GPU(Windows機+Docker)での本格学習の手順は `docker/README.md` を参照。
...
rl/          RL基盤(フェーズ6): 行動カタログ・観測エンコーダ・AEC互換env・PPO学習
docker/      GPU学習環境(Windows機+WSL2+Docker)の定義と手順
...
| 4 | ヒューリスティックbot(派閥別評価関数) | ✅ 完了 |
| 5 | 放浪部族の追加(実装+レビュー+テスト+評価関数) | ✅ 完了 |
| R | ルール完全性(圧倒カード✅ → 猫簡略化・クラフト効果) | 🚧 進行中 |
| 6 | 強化学習(PPO self-play、Windows機+Docker+GPU) | 🚧 Mac完結分は完了 |
