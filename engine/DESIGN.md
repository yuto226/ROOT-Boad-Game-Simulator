...
### 12.7 報酬シェイピング(rl/env.py, rl/ppo.py。Fable 2026-07-12)

- `vp_shaping`(12.4)に加え、`build_shaping: float = 0.0` を `RootEnv` に追加。
  EYRIE の建設欄(勅令4列目, `decree[COL_BUILD]`)にポテンシャルベースシェイピング
  (Ng et al. 1999)を掛ける。ポテンシャル φ は鳥カード枚数 b・非鳥枚数 n から
  `φ = base(b) − 0.5·n`(忠臣 LOYAL_VIZIER は鳥扱い, 7.3.4)。
...
- 毎 step: `rewards[EYRIE] += build_shaping * (shaping_gamma·φ(s') − φ(s))`。
  potential-based の定理により、これは最適方策を変えない(方策不変性)。
- 終局(勝敗確定・タイムアウトとも)で `rewards[EYRIE] += build_shaping * (0 − φ(s'))`
  としてポテンシャルをゼロ化する。γ=1 なら全 step の加算がテレスコープして
  エピソード合計は定数 `−φ(s0)` になり、終局までの方策評価そのものは歪めない。
- CLI: `rl/train.py --vp-shaping`(既定 0.0, 目安 0.01 前後)/ `--build-shaping`
  (既定 0.0, 目安 0.2 前後)。いずれも既定 0 で従来の rl/ppo.py の途中報酬 0・
  終局 ±1 のみという挙動と完全一致する。
- rl/ppo.py 側は、途中 0 埋め+終局のみ書き込みだった `_Trajectory.reward` を、
  agent 自身の意思決定点間の `env._cumulative_rewards` 差分回収に変更(13.3)。
  shaping が全て 0 のときは差分が常に途中 0・終局 ±1 になり数値互換。
...
- 適用:
  ...
  - `cobbler`: 使用済み記録→ **`CobblerMoveDecision(actor)`** を push。合法手=
    通常の移動候補(アクション消費なし)。放浪部族はブーツ消費なしの1回移動と
    解釈する(カードが移動そのものを与えるという読み。敵対広場の追加コストも
    免除=簡略化として明記)
...
## 19. ルール完全性D(第1弾): 支払い選択のデシジョン化=catalog v5(Fable 2026-07-12。実装者はこの設計に従うこと)

D のうち **catalog が上がる3項目**(木材・支援者・放浪アイテム)を1バッチで行う。
catalog 非影響の項目(工房ごとクラフト・鷲巣any割当・戒厳令確認)は 19.6 に仕様
だけ記し、次バッチとする。

### 19.1 猫の木材支払い広場選択(6.5.4)

- 現状: `_apply_build` が BFS 順に自動回収。これを選択化する。
- `apply(MarquiseBuild)`: コスト確定後、**木材を1個ずつ選ぶ**
  **`WoodPaymentDecision(actor=猫, remaining: int, build_clearing: int,
  build_kind: str)`** を push(remaining=コスト)。コスト0なら従来どおり即建設。
- 新 Action **`MarquiseChooseWood(player, clearing)`**: 候補=建設広場から連結の
  支配下広場(既存BFSと同じ集合)のうち木材が1個以上ある広場。適用=その広場の
  木材1個をサプライへ、remaining-1。remaining==0 になったら建設を完了
  (タイル配置+VP+アクション消費。現行 _apply_build 後半を流用)。
- 候補が毎回1つなら既存の単一選択自動適用(3.2)が働くので挙動上の追加分岐は不要。
- 支配は兵士+建物で決まり木材(トークン)除去では変わらないため、支払い途中で
  eligibility は変化しない(コメントに残す)。

### 19.2 連合の支援者支払い選択(8.4.1 / 8.4.2)

- 現状: `_take_supporters` が「一致suit優先→鳥」の決定的自動選択。これを、
  **一致suitと鳥の両方があるときだけ**選択化する。
- **`SupporterPaymentDecision(actor=連合, suit: Suit, remaining: int)`** を push
  (反乱=2枚、支持拡大=1〜3枚+戒厳令+1。支払い1枚ごとに再push)。
- 新 Action **`AllianceSpendSupporter(player, suit)`**: suit=実際に払うカードの
  suit(一致suit or BIRD)。同suit内のどのカードを払うかは決定的(supporters
  タプルの先頭一致)= 明記して自動のまま。
- 一致suitのみ/鳥のみしか無いときは従来の自動選択(単一選択の自動適用でも同値)。
- 支払い完了後の処理(反乱の解決・支持トークン配置)は現行フローに接続する。

### 19.3 放浪部族: 援助の任意アイテム選択+隠れ家の修理選択(9.5.4 / D.3.2)

- **援助(9.5.4)の「任意のアイテム1枚を使用」**: 現状 `_pay_any` の自動選択を
  選択化。**`VagabondPayItemDecision(actor=部族, remaining: int=1)`** + 新 Action
  **`VagabondExhaustItem(player, kind: str)`**(kind=ItemKind 8種)。候補=未使用・
  非損傷のアイテム種。**同 kind 内はかばんエリア優先で自動**(配置枠の T/X/B は
  使うとかばんへ移動し回復ボーナス(9.4.1等)を失うため、かばん優先が弱支配。明記)。
- **隠れ家(D.3.2)の「3枚まで修理」**: 損傷アイテムが3枚以下なら全修理(自動)。
  4枚以上なら **`VagabondRepairDecision(actor=部族, remaining: int)`**(remaining=3)
  + 新 Action **`VagabondRepairItem(player, kind)`**(kind 8種)。1枚ごとに再push、
  損傷が尽きるか remaining==0 で終了。**同 kind 内は表向き優先で自動**(修理は
  裏表を変えないため、表向きを直す方が弱支配。明記)。
  修理アクション(9.5.7、1枚)も同じデシジョンを remaining=1 で共用してよい。
- **T/X/B の配置枠配置(9.2.5.I / 9.5.3)は自動のまま維持**: 配置枠に置くことは
  回復・ドロー・上限の全ボーナスで弱支配(置かない利点がない)ため選択化しない
  (理由をコメントに残す。9.2.5.I は「配置できる」=任意だが実質必然)。

### 19.4 RL追随(catalog v5)

- catalog: `MarquiseChooseWood`=(型名, clearing)12 / `AllianceSpendSupporter`=
  (型名, suit)4 / `VagabondExhaustItem`=(型名, kind)8 / `VagabondRepairItem`=
  (型名, kind)8。**`CATALOG_VERSION = 5`**。
- encoder: `_DECISION_TYPES` 末尾に `WoodPaymentDecision, SupporterPaymentDecision,
  VagabondPayItemDecision, VagabondRepairDecision` を追加(既存順は不変)。
  各デシジョンは `remaining` フィールドを持つため既存の getattr("remaining")
  エンコードで自動的に観測に入る。**新規の状態フィールドはなし**(obs 追加は
  decision onehot の4次元のみ)。

### 19.5 検証と完了条件(subagent の報告に含めること)

1. selftest 新シナリオ: (a) 木材: 複数広場から選択でき、選んだ広場から減る/
   コスト0・強制1択の自動適用 (b) 支援者: 一致+鳥混在で選択が出る・片方のみ
   なら出ない・反乱2枚/戒厳令+1の枚数 (c) 援助: アイテム種の選択・かばん優先
   (d) 隠れ家: 損傷4枚で3枚選択修理・3枚以下は自動
2. `python3 -m pytest tests/ -q` 全パス+.venv で rl 系パス+ランダム4派閥
   200試合 --validate クリーン+並列/直列の決定性一致
3. catalog v5 の size 報告+20試合の全合法手カタログ解決
4. レビュー注目点(ファイル:行)

### 19.6 次バッチ仕様メモ(catalog 非影響。今回は実装しない)

- **D-2 工房ごとクラフト(4.1.1 厳密化)**: 「1ターン1クラフト」制限を撤廃。
  **各工房が1ターン1回起動のツール**(動物種=所在広場)で、ターン内の総支払い
  能力=工房の多重集合。例(ユーザー確認済み 2026-07-12): fox工房2+mouse工房1
  → fox1 のカードと fox1+mouse1 のカードを同一ターンに両方クラフト可。
  実装は鷲巣の `eyrie_payment`(止まり木の広場ID単位の使用済み記録)と同方式で、
  `MarquiseState.workshop_used: bool` を `used_workshop_clearings: Tuple[int,...]`
  に置換する。
- **D-3 鷲巣クラフトの any 割当**: `eyrie_payment` が cost の "any" を任意suitの
  未使用止まり木に割当てられるようにする(Royal Claim の any×4 対応)。
  猫・連合・部族の any 対応も同時に確認。
- **D-5 戒厳令(8.4.2.II.a)**: 現解釈「連合以外の1派閥が単独で兵士3個以上」を
  ルール原文と照合して確認のみ(コメント更新)。

## 20. 観戦APIサーバ(フェーズ7後半。Fable 2026-07-12)

対局を都度生成する同期APIサーバ(`server/`)。17章のレコーダ/ビューアはファイル
配布方式だったが、これをオンデマンドAPI化する。セッション機構(人間vs bot・
1手ずつ進行)は今回作らない。

- エンドポイント: `GET /api/models`(rl_runs配下のckpt一覧)、
  `POST /api/games`(対局を1本実行しrecordを返す)、`GET /api/games`(生成済み
  一覧)、`GET /api/games/{id}`(record取得)。CORSは`http://localhost:5173`のみ。
- `tools/record_game.py` の `run_and_record`/`build_output`/`make_policy` を
  そのまま再利用し、`{meta, map, steps}` スキーマ(17.2)を一切変更しない。
  engine/rl/tools/uiはimportのみで変更しない。
- `server/policies.py`: `make_policy` の薄いラッパ。`nn:` policy はckpt
  ロードが重い(数十MB)ため、`functools.lru_cache(maxsize=4)` で
  ckptパス+device単位に(net, spec, catalog, device)をキャッシュし、
  `NNPolicy` インスタンス自体は対局ごとに新規生成する。torch importは
  `nn:` 使用時までモジュールトップレベルで行わない(17.1と同方針)。
- `server/games.py`: 生成済み対局をプロセス内メモリ(`OrderedDict`)に
  `game_id`(uuid短縮)で保持、上限32件・超過はFIFO破棄。永続化はしない
  (プロセス再起動で消える)。
- 将来のstep API拡張点: `server/games.py` に進行中セッション(state・次の
  合法手)を追加し、`create_session`/`step_session` を別途用意する形を想定
  (今回は未実装、コメントのみ)。
- 依存追加: `pyproject.toml` の optional-dependencies に `server`
  グループ(fastapi/uvicorn/httpx)。エンジン本体の依存ゼロ方針(12.1)は
  維持(serverもoptionalに隔離)。
- テスト(`tests/test_server.py`)はtorch不要で完結(nn:policyは対象外。
  ロード部の単体は既存rlテストがカバー)。

### 20.x フロントエンド(web/)

上記APIを叩く観戦用SPA(`web/`)。Vite + React + TypeScript(素のReact、
状態管理・UIライブラリは追加しない)。`vite.config.ts` の dev server proxy
で `/api` を `http://localhost:8000` へ転送するため、フロント側の fetch は
相対パス `/api/...` で完結し、CORS設定にもポート差にも依存しない
(server側のCORS許可はdev proxyを使わない場合の保険として維持)。
- 画面: 左ペイン=対局生成フォーム(派閥2〜4行+policy選択+seed)と生成済み
  対局一覧、右ペイン=盤面SVG+ステップ再生+サイド情報(VP/手札/派閥固有状態
  等)。record({meta, map, steps})のスキーマは17.2のものをそのまま使う
  (`web/src/types.ts` に手書きの型として複製、engine/tools側は無変更)。
- コンポーネント: `Board`(盤面SVG)/`StepControls`(再生UI)/`FactionPanel`
  (サイド情報)/`GenerateForm`(対局生成フォーム)+ `GameList`(一覧)。
- `ui/viewer.html` は当面残す(単一HTMLでfile://でも動く配布物としての
  役割はSPAでは代替できないため。D&D対局読み込み用途)。
