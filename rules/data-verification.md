...
## 放浪部族関連データ【確定 2026-07-09】

放浪部族の設計セッションでFableが視覚読取・転記して確定。反映先: `engine/data/boards.json`(vagabond)・`engine/data/map_autumn.json`(forests)・`rules/vagabond.md`(追補D)。

### 派閥ボード(Root Board Front.pdf p4 視認)
- 派閥関係トラック: 無関心 →(援助1回)→ +1VP →(援助2回)→ +2VP →(援助3回)→ 同盟(+2VP)。同盟後は援助毎+2VP。敵対=戦闘での配置物除去毎+1VP、敵対広場への移動は追加M1
- 配置枠: T(お茶)・X(コイン)・B(袋) 各3枠
- 回復=3枚+表T×2 / ドロー=1枚+表X / アイテム上限=6+表B×2(いずれも法典9.4.1/9.6.2/9.6.4と一致)
- ボード面のクエスト欄に "Claim a quest and replace it."(解決後補充)の記載あり

### キャラクターカード(法典 追補D p25 視認)
| キャラ | 初期アイテム | 特別アクション(コストF1) |
|---|---|---|
| 盗賊 | M,F,T,S | 盗み: 同一広場のプレイヤーの手札からランダム1枚 |
| 修繕屋 | M,F,B,H | 日常業務: 現在広場一致カードを捨て山から回収(鳥可) |
| 森護り | M,F,C,S | 隠れ家: 3枚まで修理し即夕闇へ |

### 秋マップの樹林(7つ)
秋マップスキャンの視覚読取+経路グラフの面分割から導出(両者一致)。隣接樹林=経路1本だけを挟んで接するもの(広場の角のみで接する場合は非隣接)。
| id | 囲む広場 | 隣接樹林 |
|---|---|---|
| 0 | 0,1,2,4(北) | 1,2 |
| 1 | 0,3,4,7(北西中央) | 0,2,3 |
| 2 | 2,4,5,6,7(中央東・最大) | 0,1,4,6 |
| 3 | 3,7,8,9(南西) | 1,4 |
| 4 | 5,7,9,10(南中央) | 2,3,5 |
| 5 | 5,10,11(南東) | 4,6 |
| 6 | 5,6,11(東) | 2,5 |

### 規定の食い違い(要記録)
- **クエスト補充**: 日本語版法典9.5.5に解決後の補充の記述なし(PDF p12視認で確認)。公式ボード面の "Claim a quest and replace it." に準拠し**補充あり**を採用(2026-07-09 ユーザー確認済み)。
- **圧倒カードの捨て札リダイレクト(3.3.3の一般化)**: 日本語版法典3.3.3は「コストとして消費した圧倒カードを盤脇へ」とのみ言及するが、英語版Law 3.3.3は「圧倒カードが**捨て山に置かれるときは常に**盤の横へ」と一般則を規定する。エンジンは英語版Law準拠で全ての捨て札経路(コスト消費・夕闇の手札調整・支援者/勅令パージ等)を `mechanics.to_discard` 経由で盤脇へリダイレクトする(2026-07-11, DESIGN.md 14.3)。
