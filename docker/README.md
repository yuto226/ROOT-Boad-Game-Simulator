...
問題を回避。5章参照)。リポジトリは Windows 側 `%USERPROFILE%\root-sim`(ホームディレクトリ直下。2026-07-12にユーザーが移動)、docker は Windows の
...
git clone https://github.com/yuto226/ROOT-Boad-Game-Simulator.git %USERPROFILE%\root-sim
```
...
以下は旧WSL方式のコマンド例(参考。案Aでは %USERPROFILE%\root-sim パスで同じ compose を叩く):
